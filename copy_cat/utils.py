...
def load_dataset(dataset, train, download=False):
    # Check if dataset is from MedMNIST
    if dataset == "OrganAMNIST":
        # Use our wrapper class for OrganAMNIST
        if train:
        ...
        data = TorchvisionStyleOrganAMNIST(train=train, download=download, transform=transform)
        return data
    
    # Original code for torchvision datasets
    try:
...
class TorchvisionStyleOrganAMNIST(torch.utils.data.Dataset):
    """Wrapper for OrganAMNIST that mimics torchvision dataset interface"""
    
    def __init__(self, root='./data', train=True, transform=None, target_transform=None, download=False):
        # Map the 'train' boolean to MedMNIST 'split' parameter
        split = 'train' if train else 'test'
        
        # Initialize the underlying OrganAMNIST dataset
        self.dataset = OrganAMNIST(split=split, download=download, transform=None)
        
        # Store the transforms
        self.transform = transform
        self.target_transform = target_transform
        
        # Map MedMNIST attributes to torchvision naming convention
        self.data = self.dataset.imgs
        self.targets = self.dataset.labels.squeeze().astype(np.long)
        
        # For compatibility with some code that expects these attributes
        self.classes = list(range(11))  # OrganAMNIST has 11 classes
        self.class_to_idx = {i: i for i in range(11)}
