...
def train(lr, batch_size, epochs, dataset, architecture, exp_id=None, sequence=None,
    ...
    #For MNIST / OrganAMNIST dataset, using only 10k samples instead of 60k
    # Create a custom Subset class that preserves the data attribute
    class DataPreservingSubset(torch.utils.data.Subset):
        @property
        def data(self):
            return self.dataset.data[self.indices]

    # Use the custom Subset class instead of the standard one
    trainset = DataPreservingSubset(trainset, list(range(10000)))
    ...
    if save_freq is not None and save_freq > 0:
        ...
        save_dir = os.path.join("proof_Adam", f"{dataset}_{exp_id}")
