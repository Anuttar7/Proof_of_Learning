...
dref = np.array(np.load(f'dref/{dataset}_{form}_dref.npy'))
freq_list = [1, 2, 5, 10, 20, 50, 100, 200, 300, 400]
max_norm_e = [[[] for i in range(5)] for j in range(4)]

for i in range(5):
    for f in range(len(freq_list)):
        dist = np.array(np.load(f'dists_{form}/{dataset}_{i+1}_b{batch_size}_k{freq_list[f]}.npy'))
        for j in range(4):
