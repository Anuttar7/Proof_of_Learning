...
if __name__ == '__main__':
    ...
    parser.add_argument('--model-dir', help='path/to/the/proof', type=str, default='proof_Adam')
    parser.add_argument('--save-freq', type=int, default=20, help='frequence of saving checkpoints')
    ...
    dref = np.array(np.load(f'dref/{arg.dataset}_Adam_dref.npy'))
    ...
    for lri in range(len(lr_list)):
        for i in range(5):
            model_dir = os.path.join(arg.model_dir, f"{arg.dataset}_{i+1}_b{arg.batch_size}_a{lr_names[lri]}")
            if not os.path.exists(model_dir):
                raise FileNotFoundError(f"Model directory {model_dir} not found")
            
            # Verify the model
            print(f"Verifying model with save frequency {lr_names[lri]}, {i+1}...")
            dists = calc_dist(model_dir, lr_list[lri], arg.batch_size, arg.dataset, architecture, arg.save_freq,
                            arg.dist, arg.delta)
            
            dists = np.array(dists)
            save_path_dist = f'dists_Adam/{arg.dataset}_{i+1}_b{arg.batch_size}_a{lr_names[lri]}.npy'
