...
if __name__ == '__main__':
    ...
    for f in range(len(freq_list)):
        for i in range(5):
            ...
            # save_path_dist = f'dists_Adam/{arg.dataset}_{i+1}_b{arg.batch_size}_k{freq_list[f]}.npy'
            # np.save(save_path_dist, dists)

            dists = np.array(np.load(f'dists_Adam/{arg.dataset}_{i+1}_b{arg.batch_size}_k{freq_list[f]}.npy'))
